                ...
                <>
                  ...
                  <ContextMenuPrimitive.Content
                    ...
                    <Animated.View
                      ...
                      className="z-50 py-2 border rounded-md shadow-xl border-border/20 bg-card">
          ...
          return (
            <ContextMenuPrimitive.Item key={`loading:${id}-${item.title}-${index}`} asChild>
              <Button
                disabled
                variant="plain"
                className="justify-between h-12 gap-10 px-3 rounded-none"
...
function ContextMenuLabel(props: { children: React.ReactNode }) {
  return (
    <ContextMenuPrimitive.Label
      style={{ borderBottomWidth: StyleSheet.hairlineWidth * 1.5 }}
      className="px-3 pb-2 text-sm text-muted-foreground/80 border-border dark:text-muted-foreground">
      {props.children}
    </ContextMenuPrimitive.Label>
  );
}
...
function ContextMenuSubMenu({ title, subTitle, items }: Omit<ContextSubMenu, 'loading'>) {
  ...
  return (
    <DropdownMenu
      ...
      <Button
        variant="plain"
        className="justify-between gap-10 px-3 rounded-none"
        androidRootClassName="rounded-none"
        onLayout={onLayout}
